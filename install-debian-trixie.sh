...
# Debian/Ubuntu installer for the sdl3image Zephir extension.
# - Installs SDL3 and SDL3_image dev headers (distro pkg or source build)
...
header() {
    echo "=========================================="
    echo "SDL3_image Extension Installer (Debian/Ubuntu)"
    echo "=========================================="
    echo ""
}
...
ensure_sdl3_image_debian() {
    ...
    if pc_name="$(_sdl3image_detect)"; then
        ok "SDL3_image detected via pkg-config (${pc_name})"
        return
    fi

    step "   SDL3_image not detected. Installing..."
    require_cmd apt-get
    $SUDO apt-get update -q

    if $SUDO apt-get install -y libsdl3-image-dev; then
        :
    else
        step "   libsdl3-image-dev unavailable; building SDL3_image from source..."
        $SUDO apt-get install -y pkg-config build-essential cmake git \
            libjpeg-dev libpng-dev libwebp-dev libtiff-dev || die "Failed to install SDL3_image build prerequisites."
        ...
        cmake -S "$TMP_IMG_DIR/SDL_image" -B "$TMP_IMG_DIR/SDL_image/build" \
            ...
            || die "SDL3_image cmake configure failed."
        cmake --build "$TMP_IMG_DIR/SDL_image/build" -j"$(nproc)" || die "SDL3_image build failed."
        $SUDO cmake --install "$TMP_IMG_DIR/SDL_image/build" || die "SDL3_image install failed."
        $SUDO ldconfig || true
        export PKG_CONFIG_PATH="/usr/local/lib/pkgconfig:${PKG_CONFIG_PATH:-}"
    fi
    ...
    die "SDL3_image still not detected after install/build."
}
