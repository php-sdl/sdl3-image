...
# JetPack 6 / Ubuntu 22.04 (Jammy) installer for the sdl3image PHP extension.
#
# Ubuntu 22.04 ships neither SDL3 nor SDL3_image in apt — both are built from
...
header() {
    echo "============================================"
    echo "  SDL3_image Extension Installer (JetPack 6) "
    echo "============================================"
    echo ""
}
...
# ---------------------------------------------------------------------------
# Build SDL3_image from source (Ubuntu 22.04 has no SDL3_image in apt)
# ---------------------------------------------------------------------------

build_sdl3_image_from_source() {
    step "🏗️  Building SDL3_image from source..."
    $SUDO apt-get install -y --no-install-recommends \
        build-essential cmake git \
        >>"$LOG_FILE" 2>&1 || die "Failed to install SDL3_image build prerequisites."

    local tmp
    tmp="$(mktemp -d)"

    step "   Cloning SDL_image (main)..."
    git clone --branch main --depth 1 --recurse-submodules --shallow-submodules \
        https://github.com/libsdl-org/SDL_image "$tmp/SDL_image" \
        >>"$LOG_FILE" 2>&1 || die "Failed to clone SDL_image source."

    cmake -S "$tmp/SDL_image" -B "$tmp/SDL_image/build" \
        -DCMAKE_BUILD_TYPE=Release \
        -DSDLIMAGE_VENDORED=ON \
        -DSDLIMAGE_SAMPLES=OFF \
        >>"$LOG_FILE" 2>&1 || { show_failure_logs; die "SDL3_image cmake configure failed."; }

    cmake --build "$tmp/SDL_image/build" --parallel "$(nproc)" \
        >>"$LOG_FILE" 2>&1 || { show_failure_logs; die "SDL3_image build failed."; }

    $SUDO cmake --install "$tmp/SDL_image/build" \
        >>"$LOG_FILE" 2>&1 || die "SDL3_image install failed."
...
ensure_sdl3_image() {
    step "📚 Checking SDL3_image dependency..."

    export PKG_CONFIG_PATH="/usr/local/lib/pkgconfig:${PKG_CONFIG_PATH:-}"

    _sdl3image_detect() {
        command -v pkg-config >/dev/null 2>&1 || return 1
        for _cand in SDL3_image sdl3-image sdl3_image; do
    ...
    local pc_name
    if pc_name="$(_sdl3image_detect)"; then
        ok "SDL3_image already installed (${pc_name})"
        return
    fi

    step "   SDL3_image not found — building from source."
    build_sdl3_image_from_source

    if pc_name="$(_sdl3image_detect)"; then
        ok "SDL3_image ready (${pc_name})"
        return
    fi

    die "SDL3_image still not detected after source build. Check /usr/local/lib/pkgconfig."
}
...
step "⚙️  Configuring (--enable-sdl3image)..."
./configure --with-php-config="$PHP_CONFIG" --enable-sdl3image \
