...
# macOS installer for the sdl3image Zephir extension.
# - Detects and installs SDL3 + SDL3_image via Homebrew if missing
...
header() {
    echo "=========================================="
    echo "SDL3_image Extension Installer (macOS)"
    echo "=========================================="
    echo ""
}
...
ensure_sdl3_image_macos() {
    ...
    step "   SDL3_image not found. Installing via Homebrew..."
    brew install sdl3_image || die "Failed to install SDL3_image via Homebrew."

    if pc_name="$(_sdl3image_detect)"; then
        ok "SDL3_image installed and detected (${pc_name})"
        return
    fi

    # Accept if the header is present even without a .pc file.
    if [ -f "${brew_prefix}/include/SDL3_image/SDL_image.h" ] || \
       [ -f "${brew_prefix}/include/SDL3/SDL_image.h" ]; then
        ok "SDL3_image installed (headers found; pkg-config .pc unavailable but build will proceed)"
        return
    fi

    die "SDL3_image still not detected after install. Run: brew reinstall sdl3_image"
}
